       ...
       // if (!Ext.IsAjaxRequest)
        ...
        //{
            if (!this.IsPostBack)
            {
                try
                {
                    StatusBar1.SetStatus("Logged As: " + Session["UserLoginName"].ToString());
                    //StatusBar2.SetStatus("Login Time: " + String.Format("{0:dd-MMM-yyyy HH:mm:ss}", DateTime.Now));
                    clsGlobalSetup objGs = new clsGlobalSetup();
                    //DataSet oSS = new DataSet();
                    string SolnName = objGs.GetSlnNAmeAfterLgn();
                    string strTitleAfterLogin = objGs.GetTitleAfterLgn();
...
**/
  ...
  //      oBs = objGs.GetActualBday(ASDate);
  //      int sendMailCount = 0;
  //      string ename = "";
  ...
  //  }
    ...
    //    oBs = objGs.GetActualBday(SysDate);
    ...
    //{
    //    string comId = Session["Com_ID"].ToString();
    //    clsHRM objHRM = new clsHRM();
    //    DataSet oDSSMTPInfo = new DataSet();
    //    oDSSMTPInfo = objHRM.GetSMTPInfo(comId);
